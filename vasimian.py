...
##
global yCfgNam, yCfgFid, aCfgFid, atptFid, tpltFid, versDict
...
def presets():
  ...
  global procPref, yCfgNam, yCfgFid, aCfgFid, vCfgFid, lvsdFid, solnFid, versDict
  procPref = "ysimi"
  # yasim config xml file read input 
  yCfgFid   = procPref + '-yCfg.xml'
  ...
  ##                                     ('2017.2',                 '-v2017-2') ])
  ## 
  versDict =             OrderedDict([   ('YASIM_VERSION_32',       '-v32'),   \
                                         ('YASIM_VERSION_CURRENT',  '-vCurr'), \
                                         ('2017.2',                 '-v2017-2') ])
...
#
def vblsFromTplt():
  global yCfgNam, yCfgFid, aCfgFid, vCfgFid, lvsdFid, solnFid
  ## These vbles correspond to the elements in the config file: 
...
# Set up plots
liftPlot  = figure(plot_height=120, plot_width=128, title="Lift vs AoA",
              tools="crosshair,pan,reset,save,wheel_zoom" )

dragPlot  = figure(plot_height=120, plot_width=128, title="Drag vs AoA",
              tools="crosshair,pan,reset,save,wheel_zoom" )

lvsdPlot  = figure(plot_height=120, plot_width=128, title="L / D vs AoA",
              tools="crosshair,pan,reset,save,wheel_zoom" )

#7 #plot_i.line(x='x', 'y', source=source, line_width=3, line_alpha=0.6)
liftPlot.line( x='aoa', y='Lift-G', source=yDs, line_width=3, line_alpha=0.6)
dragPlot.line( x='aoa', y='Drag-G', source=yDs, line_width=3, line_alpha=0.6)
lvsdPlot.line( x='aoa', y='LvsD',   source=yDs, line_width=3, line_alpha=0.6)

# Set up widgets
#  Approach group
varyVa    = Slider(title="Appr IAS  Va",      value= Va, start=(0.2*Va), end=(4*Va), step=(0.2*Va))
varyAa    = Slider(title="Appr AoA  Aa",      value= Aa, start=(0.2*Aa), end=(4*Aa), step=(0.2*Aa))
varyKa    = Slider(title="Appr Thrt  Ka",      value= Ka, start=(0.2*Ka), end=(4*Ka), step=(0.1*Ka))
varyRa    = Slider(title="Appr Fuel  Ra",        value= Ra, start=(0.2*Ra), end=(4*Ra), step=(0.1*Ra))
varyFa    = Slider(title="Appr Flap  Fa",        value= Fa, start=(0.2*Fa), end=(4*Fa), step=(0.1*Fa))
#  Cruise
varyVc    = Slider(title="Cruise IAS  Vc",       value= Vc, start=(0.2*Vc), end=(4*Vc), step=(0.1*Vc))
varyHc    = Slider(title="Cruise Alt  Hc",       value= Hc, start=(0.2*Hc), end=(4*Hc), step=(0.1*Hc))
varyKc    = Slider(title="Cruise Thrt  Kc",      value= Kc, start=(0.2*Kc), end=(4*Kc), step=(0.1*Kc))
varyRc    = Slider(title="Cruise Fuel  Rc",      value= Rc, start=(0.2*Rc), end=(4*Rc), step=(0.1*Rc))
#  Wing
varyIw    = Slider(title="Wing Incidence  Iw",   value= Iw, start=(-2    ), end=(4   ), step=(0.1   ))
varyAw    = Slider(title="Wing Stall Aoa  Aw",   value= Aw, start=(0.2*Aw), end=(4*Aw), step=(0.1*Aw))
varyWw    = Slider(title="Wing Stall Width  Ww", value= Ww, start=(0.2*Ww), end=(4*Ww), step=(0.1*Ww))
varyPw    = Slider(title="Wing Stall Peak  Pw",  value= Pw, start=(0.2*Pw), end=(4*Pw), step=(0.1*Pw))
varyMb    = Slider(title="Ballast Mass  Mb",     value= Mb, start=(0.2*Mb), end=(4*Mb), step=(0.1*Mb))
varyXb    = Slider(title="Ballast Posn  Xb",     value= Xb, start=(0.2*Xb), end=(4*Xb), step=(0.1*Xb))
