...
def presets():
  ...
  #print('presets rlseKywd:', rlseKywd)
  #
  lvsdFid  = procPref + rlseToDo + '-lvsd.txt'
  drgaFid  = procPref + rlseToDo + '-drga.txt'
...
def spinYasim(tFid):
  ...
  if ( spinSmpl < 1 ) :
    ...
    if  ( sys.platform.startswith('linux')):
      os.sync()
    #p.wait()
    ##  For perforamnce two tables bypassed 
    if (0) :
      # run yasim external process to generate min IAS data table saved dataset file
    ...
    ##  spinSmpl for Yasim old releases, no 'detailed' call 
    # run yasim external process to generate LvsD data table saved dataset file
    vDatHndl = open(lvsdFid, 'w')
...
menu=['-ckpt', '-inpt', '-fgad', '-orig', '-web', '-wip'])
#
wingInci( aCfgFid)
#
# Set up plots
...
#
varyVc = Slider(bar_color='silver', width=132, title="Cruz IAS Kt   Vc", value=Vc, start=(50   ), end=(650),  step=( 1.0))
...
varyDw = Slider(bar_color='silver', width=132, title="W0 iDrag--    Dw", value=Dw, start=( 0.1 ), end=(4.0 ), step=(0.002))
varyCw = Slider(bar_color='silver', width=132, title="W0 Camb  CL0/CLs", value=Cw, start=(0.000), end=(2.00), step=(0.002))
varyCm = Slider(bar_color='silver', width=132, title="WM Camb  CL0/CLs", value=Cm, start=(0.000), end=(2.00), step=(0.002))
...
varyMb = Slider(bar_color='silver', width=132, title="Bllst Mass    Mb", value=Mb, start=(-5000), end=(5000), step=(10.0))
...
varyXb = Slider(bar_color='silver', width=132, title="Bllst Posn    Xb", value=Xb, start=(-200 ), end=(200 ), step=(0.20))
